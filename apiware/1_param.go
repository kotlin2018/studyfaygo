...
// 请求参数对结构体tag的约束指令
const (
	TAG_PARAM        = "param"    // 请求参数的 tag name
	TAG_IGNORE_PARAM = "-"        // 请求参数忽略的 tag value
	KEY_IN           = "in"       // 参数的位置
	KEY_NAME         = "name"     // 指定请求参数的名称
	...
	KEY_MAXMB        = "file"    // when request Content-Type is multipart/form-data, the max memory for body.(multi-param, whichever is greater)
...
type Param struct {
	...
	rawTagValue    	reflect.Value     		// 原始 tag value
	...
	PAIR:
		for i < len(tagValue) {
			...
			case ':':
				...
				name = strings.TrimRight(tagValue[:i], " ")
				tagValue = strings.TrimLeft(tagValue[i+1:], " ")
				hadName = true
				i = 0
			case '\\':
				i++
				...
				if tagValue[i] == '<' || tagValue[i] == '>' {
					tagValue = tagValue[:i-1] + tagValue[i:]
				} else {
					i++
				}
			case '>':
				if !hadName {
					...
					value = strings.TrimRight(tagValue[:i], " ")
				}
				values[name] = value
				break PAIR
			default:
...
func (param *Param) Raw() interface{} {
	return param.rawTagValue.Interface()
}
...
func (param *Param) makeVerifyFuncs() (err error) {
	...
	if reg, ok := param.tags[KEY_REGEXP]; ok {
		var isStrings = param.rawTagValue.Kind() == reflect.Slice
